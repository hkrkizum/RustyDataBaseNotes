...
## RQ-8a: 数値型 f64 等値比較の精度ポリシー <!-- added by checklist-apply: P-03 -->

### Decision: f64 の == 演算子による厳密比較を採用

### Rationale
- ユーザーが入力した数値はフロントエンドの Number → バックエンドの f64 に変換される
- 同一の入力値は同一の f64 表現を持つため，ユーザー視点での等値判定と一致する
- epsilon 比較は閾値の選択が恣意的になり，ユーザーの期待と乖離するリスクがある
- 0.1 + 0.2 != 0.3 のような演算結果の蓄積はフィルタ比較値（ユーザー入力）では
  発生しない。PropertyValue に格納された値とフィルタ値はともに直接入力値

### Alternatives Considered
- **epsilon 比較**: 数値演算の蓄積誤差に対応できるが，フィルタのユースケースでは
  ユーザー入力同士の比較が主であり，蓄積誤差は発生しない。過剰な複雑性
