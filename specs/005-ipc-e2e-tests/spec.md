...
- **FR-006**: E2E テストは `tauri-driver` + WebDriverIO を使用し，Tauri デスクトップアプリ全体（WebView + IPC + バックエンド）を対象として実際の UI を通じて操作を検証しなければならない（MUST）
...
- **CC-001 Data Integrity**: テスト用データベースは各テストごとに一時 SQLite ファイルとして作成・マイグレーション適用され，テスト後に削除される。本番データに影響を与えてはならない
