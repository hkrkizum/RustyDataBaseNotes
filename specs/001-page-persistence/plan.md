...
|-----------|------------|-------------------------------------|
| 自動バックアップ未実装（憲章 Principle I） | 本スライスは CRUD の最小縦断スライスであり，バックアップ・復旧機構はデータ量と運用パターンが確定する後続スライスで設計する方が適切。本スライスでは WAL モード + トランザクション保護により書き込み時の破損を防止し，DB ファイル欠損・破損時には起動時エラー報告（T048）で対応する | 本スライスでバックアップを実装すると，バックアップ先・世代管理・復元 UI など未確定の設計判断が増え，最小スライスの原則に反する |
